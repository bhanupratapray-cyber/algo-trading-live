...
cursor.execute('''
    CREATE TABLE IF NOT EXISTS top_picks (
        ticker TEXT, price REAL, rsi REAL, ma_40 REAL, ma_50 REAL, 
        ema_20 REAL, pivot REAL, verdict TEXT, scan_date TEXT
    )
''')
conn.commit()

# Expanded list of stocks to guarantee data!
...
for stock_name in tickers:
    try:
        ...
        math_signal = "WAIT"
        if latest_close > latest_ma and latest_close > latest_ema and latest_rsi < 70: math_signal = "BUY"
        elif latest_rsi < 30: math_signal = "OVERSOLD"
        ...
        # WE REMOVED THE FILTER! IT NOW SAVES EVERY STOCK IT SCANS!
        print(f"✅ Scanned: {stock_name} -> {verdict}")
        cursor.execute('''
...
conn.close()
print("\n✅ Background Scan Complete. Database saved.")
